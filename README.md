...
- **Can the agent call Codex's built-in `view_image`?** Pasted images work through the proxy, but for a text-only model the desktop client may only pass a file path (with "image content omitted") instead of the image bytes, so rely on `agent-vision see --latest` for the reliable path. The built-in `view_image` tool, however, is limited on the current Codex desktop build: the client replaces its result with `[Unsupported Image]` before it reaches the proxy. For local files, use `agent-vision see <path>` (or `agent-vision see --latest` for the last pasted image).
